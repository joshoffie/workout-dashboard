...
targetScreen.classList.remove('hidden', 'slide-in-right', 'slide-out-left', 'slide-in-left', 'slide-out-
right');
...
deleteConfirmBtn.addEventListener('click', () => { onConfirm(); hideDeleteConfirm(); }, { once:
true });
...
const allClasses = [...ANIMATION_CLASSES.happy, ...ANIMATION_CLASSES.sad,
...ANIMATION_CLASSES.calm, 'happy', 'sad', 'calm'];
...
const allSets = exercise.sets.slice().sort((a, b) => new Date(b.timestamp) - new
Date(a.timestamp));
const mostRecentDate = new Date(allSets[0].timestamp);
const currentDaySets = allSets.filter(set => isSameDay(new Date(set.timestamp),
mostRecentDate));
const previousWorkoutSet = allSets.find(set => !isSameDay(new Date(set.timestamp),
mostRecentDate));
...
const previousDaySets = allSets.filter(set => isSameDay(new Date(set.timestamp),
previousWorkoutDate));
...
if (type === 'client') {
const sortedClients = Object.values(clientsData).sort((a, b) => (a.order ?? 0) - (b.order ??
0));
...
if (sessionIndex > -1) { clientsData[selectedClient].sessions.splice(sessionIndex, 1);
saveUserJson(); renderSessions(); }
...
// --- ADD THIS ---
if (isTutorialMode) {
setTimeout(() => showTutorialTip('exerciseList', 'Tap "Bench Press" to see the data.', 40),
400);
}
}
...
showTutorialTip('comparisonBanner', 'This window shows your most current session stats
vs your previous session.', 10);
...
// ------------------ SPIRAL WIDGET LOGIC ------------------
const spiralState = {
svg: null, hitPath: null, segmentsGroup: null, markersGroup: null, timeBall: null, dateDisplay:
null,
...
currentSession = { dayStr: dayStr, timestamp: d.getTime(), sets: 0, reps: 0, volume: 0,
rawSets: [] };
}
currentSession.sets++;
...
const baseR = spiralState.START_RADIUS + (spiralState.RADIAL_PITCH * (angle / (Math.PI
* 2)));
...
const activeBtn = document.getElementById('range' + range.charAt(0).toUpperCase() +
range.slice(1));
...
document.getElementById('bgTrack1').setAttribute('d',
getFullSpiralD(spiralState.OFFSETS.sets));
document.getElementById('bgTrack2').setAttribute('d',
getFullSpiralD(spiralState.OFFSETS.reps));
document.getElementById('bgTrack3').setAttribute('d',
getFullSpiralD(spiralState.OFFSETS.vol));

document.getElementById('bgTrack4').setAttribute('d',
getFullSpiralD(spiralState.OFFSETS.wpr));
...
if (!spiralState.visibleHistory || spiralState.visibleHistory.length === 0) { updateBallToLen(0);
return;
}
...
circle.setAttribute("cx", p.x); circle.setAttribute("cy", p.y); circle.setAttribute("class",
"workout-marker");
...
tracks.sets.push(createSeg(curr.sets, next.sets, t, tEnd, spiralState.OFFSETS.sets,
baseUnit));
tracks.reps.push(createSeg(curr.reps, next.reps, t, tEnd, spiralState.OFFSETS.reps,
baseUnit));
tracks.vol.push(createSeg(curr.volume, next.volume, t, tEnd, spiralState.OFFSETS.vol,
baseUnit));
tracks.wpr.push(createSeg(curr.wpr, next.wpr, t, tEnd, spiralState.OFFSETS.wpr,
baseUnit));
});
...
const animation = path.animate([{ strokeDashoffset: len }, { strokeDashoffset: 0 }], { duration:
segment.duration * 1000, fill: 'forwards', easing: 'linear' });
...
const dateStr = d.toLocaleDateString(undefined, {weekday:'short', month:'short',
day:'numeric'});
...
const handleSpiralStart = (e) => { spiralState.isDragging = true;
spiralState.hitPath.setPointerCapture(e.pointerId); const c = getSVGC(e);
updateBallToLen(getClosestLen(c.x, c.y));
}
const handleSpiralMove = (e) => { if(!spiralState.isDragging) return; e.preventDefault(); const c
= getSVGC(e); updateBallToLen(getClosestLen(c.x, c.y));
}
const handleSpiralEnd = (e) => { if (!spiralState.isDragging) return; spiralState.isDragging =
false; spiralState.hitPath.releasePointerCapture(e.pointerId);
}
...
function getLastSet() {
if (!selectedExercise || !selectedExercise.sets || selectedExercise.sets.length === 0) return
null;
const sortedSets = selectedExercise.sets.slice().sort((a, b) => new Date(b.timestamp) - new
Date(a.timestamp));
return sortedSets[0];
}
function renderSets() {
...
const ascendingSets = selectedExercise.sets.slice().sort((a, b) => new Date(a.timestamp) -
...
<span class="set-weight-val">${s.weight}<span style="font-size:0.7em; margin-
left:2px;">lbs</span></span>
...
<textarea class="set-note-input" placeholder="Add notes..." maxlength="40"
rows="1">${currentNotes}</textarea>
...
selectedExercise.sets[originalIndex].volume =

selectedExercise.sets[originalIndex].reps * selectedExercise.sets[originalIndex].weight;
...
noteInput.addEventListener("input", (e) => { charCount.textContent =
...
return {
x: x,
yReps: maxReps ? chartState.height - ((p.reps / maxReps) * chartState.height) :
chartState.height,
yVol: maxVol ? chartState.height
- ((p.vol / maxVol) * chartState.height) : chartState.height,
yWpr: maxWpr ? chartState.height - ((p.wpr / maxWpr) * chartState.height) :
chartState.height,
ySets: maxSets ? chartState.height - ((p.sets / maxSets) * chartState.height) :
chartState.height,
data: p
};
});
...
handleInteraction(e.touches[0].clientX); }, { passive: false });
touchLayer.addEventListener('touchstart', (e) => { e.preventDefault();
handleInteraction(e.touches[0].clientX); }, { passive: false });
...
document.querySelectorAll('.pill-group .filter-btn').forEach(b =>
...
function isSameDay(d1, d2) { return d1.getFullYear() === d2.getFullYear() && d1.getMonth()
...
let changeString = `(${changeSign}${formatNum(change)} /
${changeSign}${Math.abs(percentageChange).toFixed(0)}%)`;
...
const sessionToEdit = clientsData[selectedClient].sessions.find(s => s.session_name
=== currentVal);
if (sessionToEdit) {
...
if (newVal.length > 41) {
alert(`Exercise name is too long (${newVal.length} chars).\nPlease keep it under 41

characters.`);
return;
}
// -----------------------------------
const exerciseToEdit = selectedSession.exercises.find(ex => ex.exercise ===
currentVal);
if(exerciseToEdit) {
...
break;
case "Set Reps":
selectedExercise.sets[originalIndex].reps = parseInt(newVal) ||
selectedExercise.sets[originalIndex].reps;
selectedExercise.sets[originalIndex].volume = selectedExercise.sets[originalIndex].reps *
...
document.querySelectorAll("#clientList li > span").forEach(span => makeEditable(span,
"Client"));
document.querySelectorAll("#sessionList li > span").forEach((span, idx) =>
makeEditable(span, "Session"));
document.querySelectorAll("#exerciseList li > span").forEach((span, idx) =>
makeEditable(span, "Exercise"));
// REMOVED THE SETS TABLE LOOP HERE
// Because renderSets() now handles its own edit logic internally (lines 142-160 in the code
above).
}
...
touchStartY = e.touches[0].clientY; touchMoveX = 0; touchMoveY = 0; }, { passive: true });
document.body.addEventListener('touchmove', (e) => { touchMoveX = e.touches[0].clientX;
touchMoveY = e.touches[0].clientY; }, { passive: true });
...
case SCREENS.EXERCISES: document.getElementById('backToSessionsBtn').click();
break;
case SCREENS.SETS: document.getElementById('backToExercisesBtn').click(); break;
case SCREENS.GRAPH: document.getElementById('backToSetsFromGraphBtn').click();
break;
}
...
textEl.textContent = "Install as App:\n1. Tap the menu dots (...), then tap share button
...
const plateBtn = document.querySelector(`.plate-btn[data-
...
applyTitleStyling(graphTitle, selectedExercise.exercise,
getExerciseColorData(selectedExercise));
} else {
...
setTimeout(() => showTutorialTip('touchLayer', 'Touch and drag across the graph to see
details.', 50), 1000);
}
});
};
