...
  } catch (err) {
    // Use the custom modal for errors too!
    showDeleteConfirm("Login failed: " + err.message, () => {});
    // This is a hack, we need a simple "OK" modal. For now, let's just alert.
    // alert("Login failed: " + err.message); // Re-instating alert as a fallback
  }
};
...
function showDeleteConfirm(message, onConfirm) {
  ...
  // We add { once: true } so these listeners automatically remove themselves
  // after being clicked. This prevents multiple "delete" actions from
  // stacking up.
...
      <td>${s.notes}</td>
      <td>${s.timestamp}</td>
// Note: The empty <td> for the button is added below
    `;
    
...
  if (!sets || sets.length === 0) {
    // Use our new modal for this!
    showDeleteConfirm("No sets to graph. Add some sets first!", () => {});
    // This is still a hack. A real "alert" modal is needed.
...
  
  ...
  const layout = {
    title: `${selectedExercise.exercise} Progress`,
    hovermode: 'x unified',
    paper_bgcolor: 'transparent', // Use var(--color-card)
    plot_bgcolor: 'transparent', // Use var(--color-card)
    font: {
      color: '#f0f0f0' // Use var(--color-text)
    },
    xaxis: {
      gridcolor: '#333333' // Use var(--color-border)
    },
    yaxis: {
      gridcolor: '#333333' // Use var(--color-border)
    }
...
  editToggleBtn.textContent = editMode ? "Done" : "Edit";
  ...
  // The old JS for changing color/bg is removed, CSS handles it
  if (!editMode) {
